...
-- OUTPUT_FILE: ADT.csv
---------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS :TNX_SCHEMA.n3c_adt;
...
FROM :TNX_SCHEMA.n3c_cohort n3c
	...
	LEFT JOIN :TNX_SCHEMA.encounter enc ON enc.encounter_id = adt.encounter_id
...
INSERT INTO :TNX_SCHEMA.n3c_data_counts SELECT 'ADT', count(1) FROM :TNX_SCHEMA.n3c_adt;
COMMIT;
