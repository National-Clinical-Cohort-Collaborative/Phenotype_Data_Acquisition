...
SELECT distinct '@siteAbbrev' as SITE_ABBREV,
   ...
   '@maxNumShiftDays' as MAX_NUM_SHIFT_DAYS, --maximum number of days that you are shifting dates, write UNKNOWN if you do not know.
	...
	(SELECT COUNT(*) FROM (SELECT OBSCLINID, COUNT(*) as COUNT_N 
			FROM @cdmDatabaseSchema.OBS_CLIN 
				JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_CLIN.PATID = @resultsDatabaseSchema.N3C_COHORT.PATID 
					AND OBSCLIN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
			GROUP BY OBSCLINID 
			HAVING COUNT(*) >= 2
		 ) tbl
	 )  DUP_COUNT
	...
	(SELECT COUNT(*) FROM (SELECT OBSGENID, COUNT(*) as COUNT_N 
			FROM @cdmDatabaseSchema.OBS_GEN 
				JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_GEN.PATID = @resultsDatabaseSchema.N3C_COHORT.PATID 
					AND OBSGEN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') 
				GROUP BY OBSGENID 
				HAVING COUNT(*) >= 2
		 ) tbl
	 )  DUP_COUNT
...
SELECT OBSCLINID,
   ...
   ENCOUNTERID,
   OBSCLIN_PROVIDERID,
   CAST(OBSCLIN_DATE as TIMESTAMP) as OBSCLIN_DATE,
   OBSCLIN_TIME,
   OBSCLIN_TYPE,
   OBSCLIN_CODE,
   OBSCLIN_RESULT_QUAL,
...
FROM @cdmDatabaseSchema.OBS_CLIN JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_CLIN.PATID = N3C_COHORT.PATID
  WHERE OBSCLIN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') ;
...
SELECT OBSGENID,
   ...
   ENCOUNTERID,
   OBSGEN_PROVIDERID,
   CAST(OBSGEN_DATE as TIMESTAMP) as OBSGEN_DATE,
   OBSGEN_TIME,
   OBSGEN_TYPE,
   OBSGEN_CODE,
   OBSGEN_RESULT_QUAL,
...
FROM @cdmDatabaseSchema.OBS_GEN JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_GEN.PATID = N3C_COHORT.PATID
  WHERE OBSGEN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') ;
...
(SELECT 'DEMOGRAPHIC' as TABLE_NAME,
 ...
 FROM DUAL  UNION SELECT 'ENCOUNTER'  TABLE_NAME,
  ...
  FROM DUAL  UNION SELECT 'OBS_CLIN'  TABLE_NAME,
   (SELECT count(*) FROM @cdmDatabaseSchema.OBS_CLIN JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_CLIN.PATID = N3C_COHORT.PATID AND OBSCLIN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') ) as ROW_COUNT

  FROM DUAL  UNION SELECT 'OBS_GEN'  TABLE_NAME,
   (SELECT count(*) FROM @cdmDatabaseSchema.OBS_GEN JOIN @resultsDatabaseSchema.N3C_COHORT ON OBS_GEN.PATID = N3C_COHORT.PATID and OBSGEN_DATE >= TO_DATE(TO_CHAR(2018,'0000')||'-'||TO_CHAR(01,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD') ) as ROW_COUNT
