...
create table @tempDatabaseSchema.nfixdfgqcodesets (
  codeset_id INT64 not null,
  concept_id INT64 not null
)
;

insert into @tempDatabaseSchema.nfixdfgqcodesets (codeset_id, concept_id)
select 0 as codeset_id, c.concept_id from (select distinct i.concept_id from
(
  select concept_id from @vocabularyDatabaseSchema.concept where concept_id in (706179,706166,706174,723459,706181,706177,706176,706180,706178,706167,706157,706155,706161,706175,706156,706154,706168,706163,706170,706158,706169,706160,706173,706172,706171,706165,706159,586523,586526,715272,586515,586516,586517,586518,586520,586519,586521,586522,715262,715261,715260,757686,757685,757677,757678,757679,757680)
union distinct select c.concept_id
  ...
  and ca.ancestor_concept_id in (706179,706166,706174,723459,706181,706177,706176,706180,706178,706167,706157,706155,706161,706175,706156,706154,706168,706163,706170,706158,706169,706160,706173,706172,706171,706165,706159,586523,586526,715272,586515,586516,586517,586518,586520,586519,586521,586522,715262,715261,715260,757686,757685,757677,757678,757679,757680)
...
CREATE TABLE @tempDatabaseSchema.nfixdfgqqualified_events
...
join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((m.measurement_concept_id = codesets.concept_id and codesets.codeset_id = 0))
) c
...
join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((po.procedure_concept_id = codesets.concept_id and codesets.codeset_id = 1))
) c
...
(
  ...
  join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 3))
) c
...
(
  select co.*
  from @cdmDatabaseSchema.condition_occurrence co
  join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 5))
) c
...
(
  select co.*
  from @cdmDatabaseSchema.condition_occurrence co
  join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 4))
) c
...
(
  select co.*
  from @cdmDatabaseSchema.condition_occurrence co
  join @tempDatabaseSchema.nfixdfgqcodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 2))
) c
...
create table @tempDatabaseSchema.nfixdfgqinclusion_events (inclusion_rule_id INT64,
	person_id INT64,
	event_id INT64
);

CREATE TABLE @tempDatabaseSchema.nfixdfgqincluded_events
 AS WITH cteincludedevents  as (select event_id as event_id,person_id as person_id,start_date as start_date,end_date as end_date,op_start_date as op_start_date,op_end_date as op_end_date,row_number() over (partition by person_id order by start_date asc)  as ordinal from (
     ...
     from @tempDatabaseSchema.nfixdfgqqualified_events q
    left join @tempDatabaseSchema.nfixdfgqinclusion_events i on i.person_id = q.person_id and i.event_id = q.event_id
...
CREATE TABLE @tempDatabaseSchema.nfixdfgqcohort_rows
 AS WITH cohort_ends   as (select event_id as event_id,person_id as person_id,op_end_date  as end_date from @tempDatabaseSchema.nfixdfgqincluded_events
), first_ends   as (select f.person_id as person_id,f.start_date as start_date,f.end_date
	 as end_date from (
	  select i.event_id, i.person_id, i.start_date, e.end_date, row_number() over (partition by i.person_id, i.event_id order by e.end_date) as ordinal
	  from @tempDatabaseSchema.nfixdfgqincluded_events i
...
INSERT INTO @resultsDatabaseSchema.n3c_cohort
 WITH cteenddates   as (select person_id
		...
		(
			...
			from @tempDatabaseSchema.nfixdfgqcohort_rows
	...
	where (2 * e.start_ordinal) - e.overall_ord = 0
		 ...
		 as start_date,min(e.end_date)  as end_date  from @tempDatabaseSchema.nfixdfgqcohort_rows c
...
DELETE FROM @tempDatabaseSchema.nfixdfgqcohort_rows WHERE True;
drop table @tempDatabaseSchema.nfixdfgqcohort_rows;

DELETE FROM @tempDatabaseSchema.nfixdfgqinclusion_events WHERE True;
drop table @tempDatabaseSchema.nfixdfgqinclusion_events;

DELETE FROM @tempDatabaseSchema.nfixdfgqqualified_events WHERE True;
drop table @tempDatabaseSchema.nfixdfgqqualified_events;

DELETE FROM @tempDatabaseSchema.nfixdfgqincluded_events WHERE True;
drop table @tempDatabaseSchema.nfixdfgqincluded_events;

DELETE FROM @tempDatabaseSchema.nfixdfgqcodesets WHERE True;
drop table @tempDatabaseSchema.nfixdfgqcodesets;
