...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE @resultsDatabaseSchema.N3C_PRE_COHORT  (person_id INT NOT NULL
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE @resultsDatabaseSchema.N3C_CASE_COHORT  (person_id INT NOT NULL
		,inc_dx_strong INT NOT NULL
		,inc_dx_weak INT NOT NULL
		,inc_lab_any INT NOT NULL
		,inc_lab_pos INT NOT NULL
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE @resultsDatabaseSchema.N3C_CONTROL_MAP  (case_person_id INT NOT NULL
		...
		FROM @cdmDatabaseSchema.CONDITION_OCCURRENCE
		  WHERE condition_concept_id IN (SELECT concept_id
			...
			AND condition_start_date BETWEEN TO_DATE(TO_CHAR(2020,'0000')||'-'||TO_CHAR(04,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
				AND TO_DATE(TO_CHAR(2020,'0000')||'-'||TO_CHAR(05,'00')||'-'||TO_CHAR(30,'00'), 'YYYY-MM-DD')
		-- Now we group by person_id and visit_occurrence_id to find people who have 2 or more
		GROUP BY person_id
			,visit_occurrence_id
		HAVING count(*) >= 2
...
AS (SELECT DISTINCT person_id
	...
	,cohort
AS (SELECT covid_cohort.person_id
		,CASE
 ...
 SELECT DISTINCT c.person_id
	...
	,(SELECT vocabulary_version
		FROM @cdmDatabaseSchema.vocabulary
		   ...
		   AND ROWNUM <= 1) AS vocabulary_version
FROM cohort c
...
WHERE CONTROL_PERSON_ID IN (SELECT person_id
...
DELETE
FROM @resultsDatabaseSchema.N3C_CONTROL_MAP
WHERE CONTROL_person_id NOT IN (SELECT person_id
		FROM @cdmDatabaseSchema.person
		 );
...
INSERT INTO @resultsDatabaseSchema.N3C_CONTROL_MAP (
 ...
 AS (SELECT subq.*
	...
	FROM (SELECT person_id
			...
			,hispanic
			,1 AS buddy_num
			,DBMS_RANDOM.VALUE AS randnum -- random number
		FROM @resultsDatabaseSchema.n3c_pre_cohort
		    WHERE (
				inc_dx_strong = 1
				OR inc_lab_pos = 1
		...
		SELECT person_id
			...
			,hispanic
			,2  buddy_num
			,DBMS_RANDOM.VALUE  randnum -- random number
		FROM @resultsDatabaseSchema.n3c_pre_cohort
		 WHERE (
				inc_dx_strong = 1
				OR inc_lab_pos = 1
	...
	FROM (SELECT person_id
		FROM @resultsDatabaseSchema.n3c_pre_cohort
		  WHERE inc_lab_any = 1
			...
			AND inc_lab_pos = 0
			AND inc_dx_weak = 0
			AND person_id NOT IN (SELECT control_person_id
				FROM @resultsDatabaseSchema.N3C_CONTROL_MAP
				 )
		 ) npc
...
AS (SELECT controls_1.*
	...
	FROM controls_1
	  WHERE person_id NOT IN (SELECT control_person_id
			FROM map_1
			  WHERE control_person_id IS NOT NULL
			 ) -- doesn't already have a buddy
	 )
	,map_2
AS (SELECT cases.person_id
		,cases.pt_age
		,cases.sex
		,cases.race
		,cases.hispanic
...
AS (SELECT map_1.person_id
	...
	,final_map
AS (SELECT penultimate_map.person_id AS case_person_id
