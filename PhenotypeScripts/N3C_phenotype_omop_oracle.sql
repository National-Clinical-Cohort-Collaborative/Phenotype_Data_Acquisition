...
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE @resultsDatabaseSchema.N3C_CONTROL_COHORT  (person_id INT NOT NULL
		,pt_age VARCHAR(20)
		,sex VARCHAR(20)
		,hispanic VARCHAR(20)
		,race VARCHAR(20)
...
BEGIN
  EXECUTE IMMEDIATE 'TRUNCATE TABLE @resultsDatabaseSchema.final_map';
  EXECUTE IMMEDIATE 'DROP TABLE @resultsDatabaseSchema.final_map';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
		...
		FROM @cdmDatabaseSchema.CONDITION_OCCURRENCE
		  WHERE condition_concept_id IN (SELECT concept_id
			...
			AND condition_start_date BETWEEN TO_DATE(TO_CHAR(2020,'0000')||'-'||TO_CHAR(04,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
				AND TO_DATE(TO_CHAR(2020,'0000')||'-'||TO_CHAR(05,'00')||'-'||TO_CHAR(01,'00'), 'YYYY-MM-DD')
		-- Now we group by person_id and visit_occurrence_id to find people who have 2 or more
		GROUP BY person_id
			,visit_occurrence_id
		HAVING count(distinct condition_concept_id) >= 2
