...
CREATE TABLE aa3936e2Codesets (
  codeset_id int NOT NULL,
  concept_id NUMBER(19) NOT NULL
)
;

INSERT INTO aa3936e2Codesets (codeset_id, concept_id)
...
CREATE TABLE aa3936e2qualified_events
...
JOIN aa3936e2Codesets codesets on ((m.measurement_concept_id = codesets.concept_id and codesets.codeset_id = 0))
 ) C
...
FROM (SELECT co.* 
  ...
  JOIN aa3936e2Codesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 3))
 ) C
...
FROM (SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN aa3936e2Codesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 4))
 ) C
...
FROM (SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN aa3936e2Codesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 5))
 ) C
...
FROM (SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN aa3936e2Codesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 2))
 ) C
...
JOIN aa3936e2Codesets codesets on ((o.observation_source_concept_id = codesets.concept_id and codesets.codeset_id = 6))
 ) C
...
create table aa3936e2inclusion_events (inclusion_rule_id NUMBER(19),
	person_id NUMBER(19),
	event_id NUMBER(19)
);

CREATE TABLE aa3936e2included_events
...
WITH cteIncludedEvents(event_id, person_id, start_date, end_date, op_start_date, op_end_date, ordinal)  AS (SELECT event_id, person_id, start_date, end_date, op_start_date, op_end_date, row_number() over (partition by person_id order by start_date ASC) as ordinal
  FROM (SELECT Q.event_id, Q.person_id, Q.start_date, Q.end_date, Q.op_start_date, Q.op_end_date, SUM(coalesce(POWER(cast(2 as NUMBER(19)), I.inclusion_rule_id), 0)) as inclusion_rule_mask
    FROM aa3936e2qualified_events Q
    LEFT JOIN aa3936e2inclusion_events I on I.person_id = Q.person_id and I.event_id = Q.event_id
...
CREATE TABLE aa3936e2cohort_rows

AS
WITH cohort_ends (event_id, person_id, end_date)  AS (SELECT event_id, person_id, op_end_date as end_date FROM aa3936e2included_events
 ),
...
(SELECT F.person_id, F.start_date, F.end_date
	FROM (SELECT I.event_id, I.person_id, I.start_date, E.end_date, row_number() over (partition by I.person_id, I.event_id order by E.end_date) as ordinal 
	  FROM aa3936e2included_events I
...
TRUNCATE TABLE aa3936e2cohort_rows;
DROP TABLE aa3936e2cohort_rows;

TRUNCATE TABLE aa3936e2inclusion_events;
DROP TABLE aa3936e2inclusion_events;

TRUNCATE TABLE aa3936e2qualified_events;
DROP TABLE aa3936e2qualified_events;

TRUNCATE TABLE aa3936e2included_events;
DROP TABLE aa3936e2included_events;

TRUNCATE TABLE aa3936e2Codesets;
DROP TABLE aa3936e2Codesets;
