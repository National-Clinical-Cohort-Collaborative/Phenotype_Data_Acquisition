...
CREATE TABLE lzbrauyoCodesets (
  codeset_id int NOT NULL,
  concept_id NUMBER(19) NOT NULL
)
;

INSERT INTO lzbrauyoCodesets (codeset_id, concept_id)
...
CREATE TABLE lzbrauyoqualified_events
...
JOIN lzbrauyoCodesets codesets on ((m.measurement_concept_id = codesets.concept_id and codesets.codeset_id = 0))
 ) C
...
FROM (SELECT co.* 
  ...
  JOIN lzbrauyoCodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 3))
 ) C
...
FROM (SELECT co.* 
  FROM @cdmDatabaseSchema.CONDITION_OCCURRENCE co
  JOIN lzbrauyoCodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 4))
 ) C
...
FROM (SELECT co.* 
  FROM @cdmDatabaseSchema.CONDITION_OCCURRENCE co
  JOIN lzbrauyoCodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 5))
 ) C
...
FROM (SELECT co.* 
  FROM @cdmDatabaseSchema.CONDITION_OCCURRENCE co
  JOIN lzbrauyoCodesets codesets on ((co.condition_concept_id = codesets.concept_id and codesets.codeset_id = 2))
 ) C
...
JOIN lzbrauyoCodesets codesets on ((o.observation_source_concept_id = codesets.concept_id and codesets.codeset_id = 6))
 ) C
...
create table lzbrauyoinclusion_events (inclusion_rule_id NUMBER(19),
	person_id NUMBER(19),
	event_id NUMBER(19)
);

CREATE TABLE lzbrauyoincluded_events
...
WITH cteIncludedEvents(event_id, person_id, start_date, end_date, op_start_date, op_end_date, ordinal)  AS (SELECT event_id, person_id, start_date, end_date, op_start_date, op_end_date, row_number() over (partition by person_id order by start_date ASC) as ordinal
  FROM (SELECT Q.event_id, Q.person_id, Q.start_date, Q.end_date, Q.op_start_date, Q.op_end_date, SUM(coalesce(POWER(cast(2 as NUMBER(19)), I.inclusion_rule_id), 0)) as inclusion_rule_mask
    FROM lzbrauyoqualified_events Q
    LEFT JOIN lzbrauyoinclusion_events I on I.person_id = Q.person_id and I.event_id = Q.event_id
...
CREATE TABLE lzbrauyocohort_rows

AS
WITH cohort_ends (event_id, person_id, end_date)  AS (SELECT event_id, person_id, op_end_date as end_date FROM lzbrauyoincluded_events
 ),
...
(SELECT F.person_id, F.start_date, F.end_date
	FROM (SELECT I.event_id, I.person_id, I.start_date, E.end_date, row_number() over (partition by I.person_id, I.event_id order by E.end_date) as ordinal 
	  FROM lzbrauyoincluded_events I
...
TRUNCATE TABLE lzbrauyocohort_rows;
DROP TABLE lzbrauyocohort_rows;

TRUNCATE TABLE lzbrauyoinclusion_events;
DROP TABLE lzbrauyoinclusion_events;

TRUNCATE TABLE lzbrauyoqualified_events;
DROP TABLE lzbrauyoqualified_events;

TRUNCATE TABLE lzbrauyoincluded_events;
DROP TABLE lzbrauyoincluded_events;

TRUNCATE TABLE lzbrauyoCodesets;
DROP TABLE lzbrauyoCodesets;
