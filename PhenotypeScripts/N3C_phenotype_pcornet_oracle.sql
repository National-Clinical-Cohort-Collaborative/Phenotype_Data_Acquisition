...
(SELECT distinct
        lab_result_cm.patid
    FROM lab_result_cm JOIN covid_pos_list ON LAB_RESULT_CM.RESULT_QUAL = covid_pos_list.word
      WHERE lab_result_cm.result_date >= CAST('01-JAN-2020' as TIMESTAMP)
        and 
        (
            lab_result_cm.lab_loinc in (SELECT loinc FROM covid_loinc )
...
SELECT distinct
	...
	when floor(months_between(sysdate,d.birth_date)/12) between 85 and 89 then '85-89'
        ...
        d.race as race
FROM cohort c JOIN demographic d ON c.patid = d.patid;
...
pre_controls as (
		...
		from
			n3c_pre_cohort npc JOIN encounter e ON npc.patid = e.patid
		where 
    	    inc_lab_any = 1 and inc_dx_strong = 0 and inc_lab_pos = 0 and inc_dx_weak = 0 
    	    and e.ADMIT_DATE between '01-JAN-2018' and sysdate
